...
(defun generate-wizard-session-id ()
  "Generate a unique session ID for a wizard instance. The high-entropy
   ...
   generator) so a session id can't be guessed by an attacker; the
   leading wizard-<unix-time>- prefix lets cleanup-stale-sessions
   reap by age without keeping a separate created-at field."
...
(defun process-wizard-submission (wizard-name session-id action form-data)
  ...
  (let* ((wizard-state (aif session-id
    ...
    ;; Start new wizard if no session — opens an unbound wizard for the
    ;; current user; subsequent submissions will lock it to their session.
    (unless wizard-state
           ...
           (multiple-value-bind (status result)
             ...
             (case status
               ...
               (:error
                ...
                (let ((wizard-state (get-wizard-session session-id)))
                  (htm-str
                    (:form :method "POST" :action ,route-path
